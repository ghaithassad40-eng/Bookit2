...
async function mfCall(env, path, body) {
 ...
 */
function resolveMethodId(methods, internal, currency) {
    if (internal === "any")
        return null;
    ...
    for (const m of methods) {
        ...
        if (codes.includes(code))
            exact.push(m);
        else if (codes.some((c) => code.startsWith(c)))
            startsWith.push(m);
        else if (codes.some((c) => code.includes(c)))
            contains.push(m);
    }
    const candidates = exact.length > 0 ? exact : startsWith.length > 0 ? startsWith : contains;
    if (candidates.length === 0) {
        const fb = methods.find((m) => m.PaymentMethodEn?.toLowerCase().includes(internal.replace("_", " ")));
        return fb
    ...
    const sameCurrency = candidates.find((m) => (m.PaymentCurrencyIso ?? "").toUpperCase() === currency.toUpperCase());
...
export function myfatoorahDevProxy(opts = {}) {
    return {
        ...
        configureServer(server) {
            const env = {
                ...
                MYFATOORAH_BASE_URL: opts.env?.MYFATOORAH_BASE_URL ??
                    process.env.MYFATOORAH_BASE_URL ??
                    "https://apitest.myfatoorah.com",
                MYFATOORAH_RETURN_BASE: opts.env?.MYFATOORAH_RETURN_BASE ??
                    process.env.MYFATOORAH_RETURN_BASE ??
                    "http://localhost:5173",
            };
            if (!env.MYFATOORAH_API_KEY) {
                // eslint-disable-next-line no-console
                console.warn("[bookit] MYFATOORAH_API_KEY not set in .env.local — /api/myfatoorah-* will return 503.");
            }
            else {
                // eslint-disable-next-line no-console
                console.info(`[bookit] MyFatoorah dev proxy active → ${env.MYFATOORAH_BASE_URL}`);
            }
            ...
            server.middlewares.use("/api/myfatoorah-initiate", async (req, res, next) => {
                if (req.method !== "POST")
                    return next();
                if (!env.MYFATOORAH_API_KEY)
                    return send(res, 503, { success: false, error: "MYFATOORAH_API_KEY not set" });
                try {
                    ...
                    const init = await mfCall(env, "/v2/InitiatePayment", {
                    ...
                    // eslint-disable-next-line no-console
                    if (resolved) {
                        console.info(`[bookit] MyFatoorah → method=${body.method} matched id=${resolved.id} ` +
                    ...
                    const common = {
                        ...
                        DisplayCurrencyIso: body.displayCurrency ?? body.currency,
            ...
            server.middlewares.use("/api/myfatoorah-callback", async (req, res, next) => {
                ...
                try {
                    ...
                    const status = await mfCall(env, "/v2/GetPaymentStatus", body.paymentId
